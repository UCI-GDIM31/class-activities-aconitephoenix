...
public class BatManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            BatW6 currentBat = _bats[i];
            if (Vector3.Distance(currentBat.transform.position, _playerTransform.position) < _interactDistance)
            {
                currentBat.EnableChase(_playerTransform);
            } else
            {
                currentBat.DisableChase();
            }
            
            if (Vector3.Distance(currentBat.transform.position, _playerTransform.position) < _overlapDistance)
            {
                CreateReactions(currentBat);
            }
